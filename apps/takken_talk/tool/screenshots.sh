...
#
# 撮る内容は integration_test/screenshots_test.dart にある。API_BASE 環境変数でAPIの向き先を変えられる（既定はローカル）。
#
# 撮影方法について（takken_simple の tool/screenshots.sh と同じ方式）:
# `IntegrationTestWidgetsFlutterBinding.takeScreenshot()` は使わない。iOS + Impeller では
# このAPIが実描画を無視して単色背景しか返さない不具合があり、また flutter drive の拡張
# ドライバはスクリーンショットをテスト完了後にまとめて回収するため、各ショットの
# タイミングと画面遷移が一致しない（全ショットが最後の画面になる）。
# そのため、テスト側（integration_test/screenshots_test.dart）はシミュレータの
# tmp ディレクトリにマーカーファイルを置くだけにし、このスクリプトが常駐して
# マーカーを見つけた瞬間に `xcrun simctl io screenshot` で実画面を撮る。
set -euo pipefail
...
BUNDLE_ID="jp.pairof.takken.talk"
...
flutter test integration_test/screenshots_test.dart -d "$UDID" --timeout 20m \
  --dart-define=API_BASE="${API_BASE:-http://localhost:8787}"
