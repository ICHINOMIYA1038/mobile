...
| pubspec の説明文・バージョン | 済 | `description` を実際の内容に更新済み。現在 `version: 1.0.3+4`（提出のたびに上げる） |
...
2. 関係・状況・役・種明かし・制約が表示された結果画面が出ます
   （カード右上のしおり（ブックマーク）アイコンで「お気に入り」に保存できます）
...
5. 「終了する」を押すと、演じた後の振り返り用の問いが3つと、その場面の「種明かし」が表示されます（ここにも広告が出ます）
   （「同じお題でもう一度」または「新しいお題を作る」で最初に戻れます）
6. ホーム右上のしおり（ブックマーク）アイコンから「お気に入り」一覧を確認できます
```
...
`pubspec.yaml` の `version: 1.0.3+4` が iOS の `CFBundleShortVersionString` /
...
### 2026-09-24 追記: 1.0.3(4)

- 修正: 実演タイマーを終了予定時刻からの計算に変更（スリープ中にティックが飛んで10分の
  エチュードが終わらなかった）＋ `wakelock_plus` で実演中の自動ロックを防止、終了時にハプティクス
- 修正: 1000本ぶん用意していた `secret`（場面の真相）がどこにも表示されていなかった →
  振り返り画面に「種明かし」カードとして表示
- 修正: バナー広告を画面幅で要求していたため、左右22ptの余白ぶん右端が切れていた →
  `LayoutBuilder` でスロット幅から決める
- 修正: 見出し（`title`）が「場所で、状況」の連結で100〜290字の太字段落になっていた → 場所のみ
- 修正: 役の紙を役名（大）と目的・秘密（本文）に分割。ダークテーマの `accentSoft on ink` の
  低コントラスト（1.7:1）を `onInk` に変更
- 修正: 縦画面固定（タイトル画面が横向きで溢れていた）、SKAdNetwork を1件→50件、
  お気に入り読み込み完了前の保存が消える競合、実演・振り返り画面のスワイプ戻り、
  お気に入りから実行したときの「新しいお題を作る」の戻り先、UMP 同意フォーム表示中の SDK 初期化
- 共有文の末尾に App Store の URL を追加
- ASO: アプリ名を「エチュードメーカー - 即興演劇のお題」、サブタイトルを
  「人数とジャンルを選ぶだけ。演劇部の稽古にすぐ使える」に。キーワード・説明文は `store-listing.md`
- 検索順位（2026-09-24、iTunes Search API）: 「即興演劇」1位/3件、「エチュード お題」1位/1件、
  「演劇 お題」1位/2件、「即興 お題」2位/6件、「エチュード」3位/43件（上位はピアノの練習曲アプリ）、
  「インプロ」「演劇」「即興劇」は圏外
