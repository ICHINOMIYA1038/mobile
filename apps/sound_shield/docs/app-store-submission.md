...
- **2026-09-24: v1.1.0 は審査通過済み（READY_FOR_DISTRIBUTION）だったが、App Store で検索にも
  iTunes Lookup にも出ていなかった**。原因は ASC の「アプリの配信状況（配信地域）」が未設定
  （`GET /v1/apps/{id}/appAvailabilityV2` が 404）で、ネタガチャが 2026-08-27 に踏んだのと同じ
  「App Storeから削除済み」状態。`POST /v2/appAvailabilities` で 175 地域すべてを available にして
  復旧（`contentStatuses: PROCESSING_TO_AVAILABLE` → 反映まで数時間かかることがある）。
  **新規アプリは初回承認後に必ず配信地域が入っているか `python3 scripts/asc/asc.py` と
  iTunes Lookup で確認すること。**
...
## 7. 2026-09-24 追記: 1.1.1(3)

監査で見つかった不具合の修正。ビルド・提出は ASC API（`scripts/asc/`）。

- **計測画面から戻るボタン/スワイプで抜けるとネイティブのマイクが回り続け、以降の計測が
  すべて「既に計測中です」で失敗していた**（内見診断・止めるまで測定）→ `PopScope` で必ず
  `cancelMeasurement` を通す。`dispose` でも保険で止める
- 叩きが弱く余韻が測れなかったノックを「重い壁」（最良・減点0）と判定していた → 判定不能に
- カメラ拒否時に AR マップが永遠に「計測中」で終了できなかった → ネイティブで権限確認し
  `camera_denied` を返して案内を表示。AR の X で閉じるときにネイティブ側を先に停止
  （`dispose` 任せだと UiKitView が先に消えて止まらなかった）。`deinit` でも停止
- コントロールセンターを引いただけで計測が黙って消えていた → `didEnterBackground` でのみ停止し、
  中断時はスナックバーで理由を表示
- AR の「主な侵入経路」に「部屋の中央」が出ることがあった。防振ゴムが常に「経路が違う」だった
- 校正の説明文が「内見診断には影響しない」となっていたが実際は静けさ判定と予測に効く → 文言修正。
  校正の完了メッセージが±20dB の丸め前の値を表示していた。結果画面の「未校正」表示を校正状態で切替
- PDF の環境省基準の引用（住居地域 55/45dB を根拠に 40dB としていた矛盾）を「特に静穏を要する地域の
  夜間基準 40dB」に修正
- 縦画面固定（計測・フローの固定高さレイアウトが横向きで溢れる）
- ASO: キーワードを `ios/fastlane/metadata/ja/keywords.txt` のとおり差し替え（サブタイトルと重複する
  内見/防音/診断を外し、騒音計・デシベル・遮音・壁・薄い等を追加。競合「防音チェッカー」の語も含む）
- 2026-09-24 に発覚: 1.1.0 は審査通過後も **配信地域が未設定でストアに出ていなかった**（本ファイル冒頭の
  提出状況参照）。復旧済み

未対応（次回）: マイク拒否時に設定アプリを開く導線、ノック手順でのマイク権限チェック、
中央未計測時の窓・隣室評価の扱い、ライブ表示（非A特性）と結果（A特性）の差。
