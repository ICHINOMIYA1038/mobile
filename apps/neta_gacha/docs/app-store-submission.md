...
- iOS 1.0は2026-08-26 23:19に審査通過（「配信準備完了」）
- **ただし審査通過後、「価格および配信状況」の「アプリの配信状況」（配信国・地域）が未設定になっており、
  アプリ一覧で「App Storeから削除済み」表示・ストアに出ていない状態だった**（8/20に175か国で設定した
  つもりが保存されていなかった模様）。2026-08-27に「すべての国または地域」(175)で再設定済み。
  反映まで最大24時間。再審査は不要
- 教訓: 審査提出後は必ず「価格および配信状況」ページで配信国が入っているか確認すること
- 以下は2026-08-20時点の記録
...
## 2026-09-24 追記: 1.0.1(2)

監査で見つかった不具合の修正と ASO 更新。ビルド・提出は ASC API（`scripts/asc/`）で実施。

- 配信前リマインダーが8日目から鳴らなくなっていた（7日ぶんの個別予約で、設定画面を開き直さない限り
  再予約されない）→ 毎日繰り返す1件の予約（`matchDateTimeComponents: time`）＋起動時に再予約。
  `tz.local` が UTC のままだったのを端末オフセットのロケーションに
- 通知を拒否されてもスイッチが ON になっていた → 端末側の許可を確認し、拒否時は設定アプリへ案内
- 全画面広告をその場で読み込んでいたため、数秒後にお題の上や戻った先のホームに被さっていた →
  抽選画面を開いた時点で事前読み込みし、演出の後にその画面が表示中のときだけ出す
- 時刻ピッカー等が英語（`flutter_localizations` 未設定）→ 日本語化
- 縦画面固定（抽選画面が横向きで溢れる）、SKAdNetwork 1件→50件、文字サイズ大でホームのタイルが切れる、
  タイマーがバックグラウンドで止まる、タイマーのボタン行の溢れ、カスタムお題の空白/重複/長さ、
  設定画面の dispose 後 setState、EEA の同意見直し導線
- 画面内・通知・共有ハッシュタグの「配信ネタガチャ」をストア名「ネタガチャ」に統一
- ストア文言の「NGワード」は実装（6テーマのオン/オフ）と違っていたので「テーマ非表示」に修正
