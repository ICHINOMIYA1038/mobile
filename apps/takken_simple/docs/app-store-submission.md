...
- **名前**: シンプルに学ぶ宅建
- **サブタイトル**: 配点順に出る一問一答・合格グラフ（1.0.1〜。1.0 は「配点順に出る宅建の一問一答」で、アプリ名と重複する「宅建」を外した）
- **カテゴリ**: 教育（第2カテゴリ: 参考書）
- **年齢制限**: 4+

### App のプライバシー（申告内容）

学習履歴そのものは端末内のみで外部送信がない。ただし **AdMob（広告）と Firebase
Analytics / Crashlytics（`packages/app_insights`、2026-08〜）を組み込んでいるため、SDK が扱う分を
申告する必要がある**。

- トラッキング: **なし**（非パーソナライズ広告のみ配信し、ATT も要求していない）
- 収集するデータ:
  - **識別子 → デバイスID**: 「サードパーティ広告」目的、**ユーザーに紐付けない**（AdMob）
  - **使用状況データ → 広告データ**: 「サードパーティ広告」目的、**ユーザーに紐付けない**（AdMob）
  - **使用状況データ → 製品の操作**: 「分析」目的、**ユーザーに紐付けない**（Firebase Analytics。
    画面遷移・セッション開始/終了・正誤と分野。どの問題に何と答えたかは送らない）
  - **診断 → クラッシュデータ**: 「分析」目的、**ユーザーに紐付けない**（Crashlytics）
- 学習履歴（どの問題に何と答えたか、復習予定、連続日数）は端末内のみで、申告対象外。

**2026-09-24時点の注意**: ASC の申告は Firebase 導入前（AdMob のみ）のままの可能性が高い。
1.0.1 の提出前に App Store Connect →「App のプライバシー」で上記2項目（製品の操作・クラッシュ
データ）が入っているか確認し、無ければ追加すること。アプリ内の文言（成績画面「学習データ」）と
`docs/privacy-policy.md`（公開済み）は 2026-09-24 に揃えた。
...
多く出題します（成績画面の「合格グラフ」のマス目の数も配点比率に対応しています）。
...
`pubspec.yaml` の `version: 1.0.1+5` が iOS の
...
| 日付 | 内容 |
| --- | --- |
| 2026-07-23 | 1.0 提出 → 2026-07-24 公開 |
| 2026-09-24 | 1.0.1(5) 準備・提出。復習優先の出題（スケジューラが未学習を復習より優先していた根本バグ）、スワイプ戻り時のセッション終了、連続日数の失効表示、復元の終了状態、中区分の振り分け、文字サイズ追従、苦手モードの重複出題、通知許可の案内、用語の自己リンク、広告初期化の例外処理。ASO: サブタイトル・キーワード・説明文を刷新（`docs/store-listing.md`）。**本試験 2026-10-18 に間に合わせるため最優先で提出** |



## 2026-09-26 SKAdNetwork の識別子を50件に揃えた

このアプリだけ `SKAdNetworkItems` に Google 本体の `cstr6suwn9.skadnetwork` しか入っておらず、
広告を出す他の4アプリ（ネタガチャ・ネタメーカー・エチュードメーカー・コロコロスロープ）は50件あった。
AdMob の需要パートナー経由のインストールが計測されず、その分 eCPM が下がる。
neta_gacha の一覧をそのまま移した。次のバージョンから反映される。

広告を出すアプリを追加したときは、必ず同じ50件を入れること。確認は:
```sh
grep -c SKAdNetworkIdentifier apps/<app>/ios/Runner/Info.plist
```
