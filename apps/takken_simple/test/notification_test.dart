...
import 'package:takken_simple/data/review_prompt_service.dart';
...
class _FakeReviews implements ReviewPromptService {
  int requests = 0;
...
void main() {
  ...
  group('通知設定', () {
    test('オフにすると予約を消し、学習終了時にも組み直さない', () async {
    ...
    test('設定は再起動後も残る', () async {
      ...
      expect(revived.remindersEnabled, isFalse);
    });
  });
  ...
  group('評価依頼', () {
    test('30問・3日継続後に一度だけ依頼する', () async {
      ...
      expect(await eligible.maybeRequestReview(), isTrue);
      expect(await eligible.maybeRequestReview(), isFalse);
  ...
  group('何件と数えるか', () {
    ...
    ReviewState answered({required String id, required DateTime dueAt}) =>
        ReviewState(
          questionId: id,
          repetition: 1,
          intervalDays: 1,
    ...
    test('期限が来た問題だけを数える', () {
      ...
      expect(
        service.dueCountAt(questions: questions, states: states, at: now),
        2,
      );
    });
