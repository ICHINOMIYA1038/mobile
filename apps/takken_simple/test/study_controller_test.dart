...
void main() {
  ...
  test('間違えた問題だけで復習セッションを開始できる', () async {
    controller.startSession();
    final weakQuestion = controller.current!;
    await controller.answer(!weakQuestion.answer);

    expect(controller.weakQuestions.map((q) => q.id), [weakQuestion.id]);
    controller.startWeakSession();
    expect(controller.current?.id, weakQuestion.id);

    await controller.answer(weakQuestion.answer);
    expect(controller.weakQuestions, isEmpty);
  });
  ...
  group('合格グラフ', () {
    ...
    test('正解するとその科目のマスが1つ濃くなる', () async {
      ...
      final stats = controller.categoryStats.firstWhere(
        (s) => s.category.label == question.category,
      );
      expect(
    ...
    test('間違えるとそのマスは要復習として区別される', () async {
      ...
      expect(
        stats.levels.where((l) => l == MasteryLevel.needsReview).length,
        1,
      );
  ...
  test('正答率は回答済みの中での割合を示し、未回答に引きずられない', () async {
    ...
    final answeredStats = controller.categoryStats.firstWhere(
      (s) => s.answered > 0,
    );
